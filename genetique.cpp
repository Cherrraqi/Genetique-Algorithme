...
void fitness_fonction(deque<float> &solution_candidate, deque<float> &fitness){
      ...
      float tmp = pow(sin(solution_candidate[i] *10),2)/(solution_candidate[i]+1);
...
void mielleur_individue(deque<float> &solution_candidate,deque<float> &fitnes,deque<float>  &decendant){    
     ...
     int nb = solution_candidate.size();
     fitness_fonction(decendant, fitnes_1);
     /*initialisation du pop par solution_candidate et decendant*/
     for(int i=0;i<decendant.size();i++){
       solution_candidate.push_back(decendant[i]);}
     /*        */
     for(int i=0;i<fitnes_1.size();i++){
       fitnes.push_back(fitnes_1[i]);}
     decendant.clear();
     do
     {
     t++;
     int indice_1 = (max(fitnes.begin(),fitnes.end())-fitnes.begin());
     decendant.push_back(solution_candidate[indice_1]);
     fitnes.erase(fitnes.begin()+indice_1);    
     }while(t < nb);
     fitnes.clear();
